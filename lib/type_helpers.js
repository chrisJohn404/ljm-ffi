...
var ARCH_CHAR_NUM_BYTES = ref.types.char.size;
var ARCH_INT_NUM_BYTES = ref.types.int.size;
var ARCH_DOUBLE_NUM_BYTES = ref.types.double.size;
var ARCH_POINTER_SIZE = ref.types.CString.size;
...
ljTypeOps['char**'] =  {
    ...
    'parse': function(buffer) {
        var pointerToCharStar = ref.get(buffer,0,'pointer');
...
ljTypeOps['a-char**'] = {
    ...
    'parse': function(buffer) {
        ...
        var offset = 0;
        for ( i = 0; i < length; i ++) {
            var buf = ref.get(buffer,offset,'pointer');
